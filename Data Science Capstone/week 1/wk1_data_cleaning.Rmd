...
get_info <-lapply(paste("Data", flist, sep = "/"), function(filename){
      ...
      close(con)
      return(c(filename, fsize, nlines, longest_line))
})
...
```{r df, echo=FALSE}
df <- data.frame(matrix(unlist(get_info), nrow=length(get_info), byrow=T))
colnames(df) <- c("file", "fsize", "num of lines", "longest line")
df
```
